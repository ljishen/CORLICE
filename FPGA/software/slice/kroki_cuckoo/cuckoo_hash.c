    ...
    if (*h1 != *h2) {
        ...
        *h2 = ~*h2;
        ...
        XPROBES_SITE(cuckoo_hash, compute_hash_equal,
                (const void *, size_t, uint32_t), (key, key_len, *h1));
    }
}
...
bool cuckoo_hash_init(struct cuckoo_hash *hash, unsigned char power) {
    ...
    hash->bin_size = 4;
    hash->count = 0;
    hash->table = calloc((size_t) hash->bin_size << power,
            sizeof(*hash->table));

    if (!hash->table)
        return false;

    XPROBES_SITE(cuckoo_hash, init, (const struct cuckoo_hash *), (hash));

    return true;
}

void cuckoo_hash_destroy(const struct cuckoo_hash *hash) {
    XPROBES_SITE(cuckoo_hash, destroy, (const struct cuckoo_hash *), (hash));
...
static inline struct cuckoo_hash_item *lookup(const struct cuckoo_hash *hash,
    ...
    while (elem != end) {
        if (elem->hash2 == h2 && elem->hash1 == h1
            ...
            XPROBES_SITE(cuckoo_hash, lookup_hash1, (const struct cuckoo_hash *, int),
    ...
    while (elem != end) {
        if (elem->hash2 == h1 && elem->hash1 == h2
                && elem->hash_item.key_len == key_len
                && memcmp(elem->hash_item.key, key, key_len) == 0) {
            XPROBES_SITE(cuckoo_hash, lookup_hash2, (const struct cuckoo_hash *, int),
    ...
    XPROBES_SITE(cuckoo_hash, lookup_not_found, (const struct cuckoo_hash *, int),
    ...
    if (hash_item) {
        struct _cuckoo_hash_elem *elem =
                ((struct _cuckoo_hash_elem *) ((char *) hash_item
                        - offsetof(struct _cuckoo_hash_elem, hash_item)));
        elem->hash1 = elem->hash2 = 0;
        --hash->count;

        XPROBES_SITE(cuckoo_hash, remove, (const struct cuckoo_hash *), (hash));
    }
}
...
static bool undo_insert(struct cuckoo_hash *hash,
        struct _cuckoo_hash_elem *item, size_t max_depth, uint32_t offset,
        int phase) {
    uint32_t mask = (1U << hash->power) - 1;
    ...
    for (size_t depth = 0; depth < max_depth * phase; ++depth) {
        ...
        if (h1m != h2m) {
            ...
            XPROBES_SITE(cuckoo_hash, insert_undo_inserted,
                    (const struct cuckoo_hash *, int, size_t, size_t),
    ...
    XPROBES_SITE(cuckoo_hash, insert_undo,
            (const struct cuckoo_hash *, int, size_t),
...
static inline bool insert(struct cuckoo_hash *hash,
    ...
    if (grow_bin_size(hash)) {
        uint32_t mask = (1U << hash->power) - 1;
        struct _cuckoo_hash_elem *last = bin_at(hash, (item->hash1 & mask) + 1)
                - 1;
...
struct cuckoo_hash_item *cuckoo_hash_insert(struct cuckoo_hash *hash,
    ...
    if (item) {
        XPROBES_SITE(cuckoo_hash, insert_exists, (const struct cuckoo_hash *),
                (hash));

        return item;
    }

    struct _cuckoo_hash_elem elem = { .hash_item = { .key = key, .key_len =
            key_len, .value = value }, .hash1 = h1, .hash2 = h2 };
...
cuckoo_hash_next(const struct cuckoo_hash *hash,
    ...
    struct _cuckoo_hash_elem *elem =
            (hash_item != NULL ?
                    ((struct _cuckoo_hash_elem *) ((char *) hash_item
                            - offsetof(struct _cuckoo_hash_elem, hash_item)) + 1) :
                    hash->table);
